...
public class OkHttpUtils {
    ...
    public void post(Context context, String url, Map<String, String> params, IResponseHandler responseHandler) {
        ...
        Request request;
        if (context == null) {
            request = (new okhttp3.Request.Builder()).url(url).post(builder.build()).build();
        } else {
            request = (new okhttp3.Request.Builder()).url(url).post(builder.build()).tag(context).build();
        }
    ...
    public void get(Context context, String url, Map<String, String> params, IResponseHandler responseHandler) {
        ...
        if (context == null) {
            ...
            request = (new okhttp3.Request.Builder()).url(url).tag(context).build();
        }
    ...
    public void upload(Context context, String url, Map<String, String> params, Map<String, File> files, IResponseHandler responseHandler) {
        ...
        Request request;
        if (context == null) {
            request = (new okhttp3.Request.Builder()).url(url).post(new ProgressRequestBody(multipartBuilder.build(), responseHandler)).build();
        } else {
            request = (new okhttp3.Request.Builder()).url(url).post(new ProgressRequestBody(multipartBuilder.build(), responseHandler)).tag(context).build();
        }
    ...
    private class MyCallback implements Callback {
        ...
        public void onFailure(Call call, final IOException e) {
            this.mHandler.post(new Runnable() {
                public void run() {
                    MyCallback.this.mResponseHandler.onFailure(0, e.toString());
                }
            });
        }
            ...
            if (response.isSuccessful()) {
                ...
                if (this.mResponseHandler instanceof JsonResponseHandler) {
                    try {
                        ...
                        this.mHandler.post(new Runnable() {
                            public void run() {
                                ((JsonResponseHandler) MyCallback.this.mResponseHandler).onSuccess(response.code(), jsonBody);
                            }
                        });
                    } catch (JSONException var5) {
                        this.mHandler.post(new Runnable() {
                            public void run() {
                                MyCallback.this.mResponseHandler.onFailure(response.code(), "fail parse jsonobject, body=" + response_body);
                            }
                        });
                    }
                } else if (this.mResponseHandler instanceof FastjsonResponseHandler) {
                    this.mHandler.post(new Runnable() {
                        public void run() {
                            try {
                                FastjsonResponseHandler responseHandler = ((FastjsonResponseHandler) MyCallback.this.mResponseHandler);
                                responseHandler.onSuccess(response.code(), JSON.parseObject(response_body, responseHandler.getParamClass()));
                    ...
                    this.mHandler.post(new Runnable() {
                        public void run() {
                            ((RawResponseHandler) MyCallback.this.mResponseHandler).onSuccess(response.code(), response_body);
                        }
                    });
                }
            } else {
    ...
    private class MyDownloadCallback implements Callback {
        ...
        public void onFailure(Call call, final IOException e) {
            this.mHandler.post(new Runnable() {
                public void run() {
                    MyDownloadCallback.this.mDownloadResponseHandler.onFailure(e.toString());
                }
            });
        }
        ...
        public void onResponse(Call call, final Response response) throws IOException {
            if (response.isSuccessful()) {
                ...
                try {
                    ...
                    this.mHandler.post(new Runnable() {
                        public void run() {
                            MyDownloadCallback.this.mDownloadResponseHandler.onFailure("onResponse saveFile fail." + var5.toString());
                        }
                    });
                    return;
                }
                ...
                this.mHandler.post(new Runnable() {
                    public void run() {
                        MyDownloadCallback.this.mDownloadResponseHandler.onFinish(finalFile);
                    }
                });
            } else {
                this.mHandler.post(new Runnable() {
